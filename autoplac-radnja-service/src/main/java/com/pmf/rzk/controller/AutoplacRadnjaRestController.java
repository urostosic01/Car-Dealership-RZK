...
//	}
	...
	public String avgPrice() {
		...
		for (Automobil a : sviAutomobili) {
			sumaCena += a.getCena();
			sumaGod += a.getGodiste(); 
		}
		
		double prosekCena = (double) sumaCena / sviAutomobili.size();
		double prosekGod = (double) sumaGod / sviAutomobili.size();
		
		return "Prosecna cena: " + String.format("%.2f", prosekCena) + "e \nProsecno godiste: " + Math.round(prosekGod);
	}
